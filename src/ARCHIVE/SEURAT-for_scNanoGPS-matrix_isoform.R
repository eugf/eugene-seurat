...
# NOTES:
# - SCRIPT PATH: /vf/users/CARDPB/data/snRNA_longread/eugene-seurat/src/SEURAT-for_scNanoGPS-matrix_isoform.R
...
input_dir <- file.path('/vf/users/CARDPB/data/snRNA_longread/eugene-seurat/output/long_reads/SH-04-08/20241122', 'isoform')
...
#* TODO - make this be an argparse user-input option
# Define output path
output_path <- file.path(input_dir, 'PLOTS-Seurat-matrix_isoform')
...
# NOTE: use col 1 = the gene_id col (use the unique ENST IDs, instead of the gene_name col)
# scNanoGPS.data <- Read10X(data.dir = file_path)
scNanoGPS.data <- Read10X(data.dir = input_dir, gene.column = 1)

# # Initialize the Seurat object with the raw (non-normalized data).
# # pbmc <- CreateSeuratObject(counts = pbmc.data, project = "pbmc3k", min.cells = 3, min.features = 200)
# NOTE: 
# `assay` --> just to give it a name, otherwise it assumes RNA by default
# `min.features > 191` --> is too high, will cause the error below:
# Warning: Data is of class numeric. Coercing to dgCMatrix.
...
# `min.features <= 191` --> is fine for the isoforms, or leave out the arg entirely

scNanoGPS <- CreateSeuratObject(
  counts  = scNanoGPS.data, 
  project = paste0(id, '-isoform'),
  assay   = 'isoform')
...
#! ERROR - with the `percent.mt`(all 0s)
...
plot1 <- FeatureScatter(scNanoGPS, feature1 = "nCount_isoform", feature2 = "percent.mt")
plot2 <- FeatureScatter(scNanoGPS, feature1 = "nCount_isoform", feature2 = "nFeature_isoform")
plot1 + plot2
...
# # CHECK
# unique(unlist(scNanoGPS@meta.data[["percent.mt"]]))
# cat('unique percent.mt values: \n', unique(unlist(scNanoGPS@meta.data[["percent.mt"]] )) , '\n')
...
# # SHAHROZE - 2000 is not enough, use 5000 - 10,000
# scNanoGPS <- FindVariableFeatures(scNanoGPS, selection.method = "vst", nfeatures = 2000)
# scNanoGPS <- FindVariableFeatures(scNanoGPS, selection.method = "vst", nfeatures = 5000)
...
# #* NOTE - all `percent.mt` = 0%, won't do anything, comment out
...
# SHAHROZE - could use 5 or up to 10
...
cluster2.markers <- FindMarkers(scNanoGPS, ident.1 = 2)
...
# # TEST - Cory
# genesymbols <- read.csv('/home/fonge2/scNanoGPS/eugene-seurat/output/TEST/isoform/genes.tsv', sep='\t', header=F)$V2
# scNanoGPS[['isoform']] <- AddMetaData(object=scNanoGPS[['isoform']], metadata=genesymbols, col.name='genesymbol')
# # TEST - me
# scNanoGPS[['isoform']] <- AddMetaData(object=scNanoGPS[['isoform']], metadata=scNanoGPS.data2, col.name='genesymbol')

# #* TODO - test all the gene lists
# VlnPlot(scNanoGPS, features = c("MS4A1", "CD79A"))
# VlnPlot(scNanoGPS, features = genes_list)

# # CHECK - use top 10 genes list
# VlnPlot(scNanoGPS, features  = top10)

# TRY
#! WARNING - huge amt of output here, can I sink it?
# CHECK
...
# # CHECK
# unique(scNanoGPS@assays[["isoform"]]@meta.data[["var.features"]])
